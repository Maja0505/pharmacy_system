...
const SheduleAppointment = ({ pharmacyInfo }) => {
  ...
  useEffect(() => {
    setPatientInfo(JSON.parse(localStorage.getItem("PatientForReport")));

    if (JSON.parse(localStorage.getItem("PatientForReport")) === null) {
      return;
    }
  ...
  return (
    <>
      {JSON.parse(localStorage.getItem("PatientForReport")) === null && (
        <Redirect to="/dermatologist" />
      )}
      {JSON.parse(localStorage.getItem("PatientForReport")) !== null && (
        <div>
